...
def load_words_from_file(default=False):
    ...
    if default and os.path.exists(file_name):
        ...
        file_path = filedialog.askopenfilename(filetypes=[("Szövegfájlok", "*.txt")])
        if file_path:
...
def update_word_list(word_list):
    ...
    rows = [word_list[i:i + max_words_per_row] for i in range(0, len(word_list), max_words_per_row)]
    for row in rows:
        row_frame = tk.Frame(word_frame)
        row_frame.pack(fill=tk.X)
        for word in row:
            btn = tk.Button(row_frame, text=word, command=lambda w=word: select_word(w), width=cell_width // 10)
...
def filter_words_by_length(event=None):
    ...
    try:
        ...
        if not length:
            filtered_words = [
                word for word in words if all(char not in word for char in exclude_chars)
            ]
        else:
            length = int(length)
            filtered_words = [
                word for word in words
                if len(word) == length and all(char not in word for char in exclude_chars)
            ]
            if not filtered_words:
                messagebox.showinfo("Info", "Nincs találat.")
        ...
        messagebox.showerror("Error", "Érvénytelen hossz!")
        length_entry.focus_set()
...
def real_time_search(event=None):
    ...
    if not length_entry.get():
        filtered_words = [
            word for word in words if word.startswith(search_term) and all(char not in word for char in exclude_chars)
        ]
    else:
        length = int(length_entry.get())
        filtered_words = [
            word for word in filtered_words
            if word.startswith(search_term) and len(word) == length
...
def apply_pattern(event=None):
    ...
    if not selected_word:
        messagebox.showerror("Error", "Válassz egy szót előbb!")
        return

    if not valid_pattern(pattern):
        messagebox.showerror("Error", "Érvénytelen mintázat. Csak '+', '-', '.' karakterek megengedettek.")
        return

    filtered_words = [word for word in filtered_words if matches_pattern(word, selected_word, pattern)]
    update_word_list(filtered_words)
...
def add_new_word(new_word):
    ...
    if new_word_cleaned and new_word_cleaned not in words:
        ...
        messagebox.showerror("Error", "Az új szó üres vagy már létezik.")
...
loaded_label = tk.Label(root, text="Betöltött fájl")
loaded_label.pack()
...
file_button = tk.Button(root, text="Tallózás...", command=lambda: load_words_from_file(default=False))
file_button.pack()

search_label = tk.Label(root, text="Keresés:")
search_label.pack()
...
exclude_label = tk.Label(root, text="Kizáró karakterek:")
exclude_label.pack()

exclude_entry = tk.Entry(root)
exclude_entry.pack()

length_label = tk.Label(root, text="Add meg a kívánt szó hosszúságot")
length_label.pack()
...
filter_button = tk.Button(root, text="Szűrés hossz alapján", command=filter_words_by_length)
filter_button.pack()

pattern_label = tk.Label(root, text="Add meg a mintát (+ - .)")
pattern_label.pack()
...
selected_word_text = tk.Label(root, text="Kiválasztott szó", font=("Liberation Sans", 12, "bold"))
selected_word_text.pack()

selected_word_display = tk.Label(root, text="", font=("Liberation Sans", 12, "bold"))
selected_word_display.pack()

confirm_button = tk.Button(root, text="Megoldás", command=confirm_solution)
confirm_button.pack()

add_word_label = tk.Label(root, text="Új szó hozzáadása:")
add_word_label.pack(pady=(10, 0))

add_word_entry = tk.Entry(root)
add_word_entry.pack()

add_word_button = tk.Button(root, text="Hozzáadás", command=lambda: add_new_word(add_word_entry.get()))
